import os
from lib.requests import update_bd
def linha(tam) -> str:
    return '=' * tam
    ...
    while alterar.upper() == 'N':
        ...
        if alterar.upper() == "N":
            ...
            match opc_alterar:
                case 1:
                    if tipo_cadastro == 'DADOS PESSOAIS':
                        update_bd('cliente', 'nome', nv_dado)
                    elif tipo_cadastro == 'ENDEREÇO':
                        update_bd('endereco', 'numero', nv_dado)
                    elif tipo_cadastro == 'DADOS BIKE':
                        update_bd('bike', 'marca', nv_dado)
                    else:
                        update_bd('acessorio', 'tipo_acessorio', nv_dado)
                case 2:
                    if tipo_cadastro == 'DADOS PESSOAIS':
                        update_bd('cliente', 'email', nv_dado)
                    elif tipo_cadastro == 'ENDEREÇO':
                        update_bd('endereco', 'logradouro', nv_dado)
                    elif tipo_cadastro == 'DADOS BIKE':
                        update_bd('bike', 'modelo', nv_dado)
                    else:
                        update_bd('acessorio', 'marca_acessorio', nv_dado)
                case 3:
                    if tipo_cadastro == 'DADOS PESSOAIS':
                        update_bd('cliente', 'telefone', nv_dado)
                    elif tipo_cadastro == 'ENDEREÇO':
                        update_bd('endereco', 'complemento', nv_dado)
                    elif tipo_cadastro == 'DADOS BIKE':
                        update_bd('bike', 'chassi', nv_dado)
                    else:
                        update_bd('acessorio', 'modelo_acessorio', nv_dado)
                case 4:
                    if tipo_cadastro == 'DADOS PESSOAIS':
                        update_bd('cliente', 'cpf', nv_dado)
                    elif tipo_cadastro == 'ENDEREÇO':
                        update_bd('endereco', 'bairro', nv_dado)
                    elif tipo_cadastro == 'DADOS BIKE':
                        update_bd('bike', 'valor', nv_dado)
                    else:
                        update_bd('acessorio', 'valor_acessorio', nv_dado)
                case 5:
                    if tipo_cadastro == 'DADOS PESSOAIS':
                        update_bd('cliente', 'cep', nv_dado)
                    else:
                        update_bd('endereco', 'localidade', nv_dado)
                case 6:
                    update_bd('endereco', 'uf', nv_dado)
        if alterar.upper() == "S":
            os.system("cls")
    return d
