...
{linha(50)}""")
        ...
        case 4:
            ...
            if d_pessoais_cadastrados:
                exibir_dados('DADOS PESSOAIS', select_bd('cliente'))
                exibir_dados('ENDEREÇO', select_bd('endereco'))
                sleep(1.0)
            else:
                dados_restantes.append('Cadastrar dados pessoais')
            if d_bike_cadastrados:
                exibir_dados('DADOS BIKE', select_bd('bike'))
                sleep(0.7)
                if possui_acessorio.upper() == 'S':
                    for i in range(quant_acessorio):
                        exibir_dados(f'ACESSÓRIO {i + 1}', select_bd_acessorios(i))
                        sleep(0.7)
            else:
